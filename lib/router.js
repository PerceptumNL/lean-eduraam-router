...
const ROUTER_BASE_DOMAIN = (process.env.ROUTER_BASE_DOMAIN || 'codecult.nl');
...
function get_routed_url(url, conf){
  ...
  if( check_domain_suffix(parsed_url.host()) ){
    parsed_url
      .scheme(conf.router_protocol)
      .domain(conf.router_base_domain)
      .subdomain(Buffer(parsed_url.host()).toString('hex'));
  }

  if( conf.token && parsed_url.host() === conf.routed_app_host ){
